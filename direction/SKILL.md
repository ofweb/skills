...
Use `references-document` to record a relevant external link during the
discussion, whether the user supplied it or investigation found it. Keep a
short reason it matters. Preserve useful user links even when their effect on
Direction remains unclear. Mark unverified claims instead of dropping the
link.
...
- Relevant external link and its context: References.
