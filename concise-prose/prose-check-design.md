...
input. CommonMark markup, code blocks, and inline code are not STE input. Full
...
The command checks all inputs with STE100 first. It runs Vale only if all inputs
pass STE100. The command returns each checker's native findings. Vale and
STE100 errors block completion. The model also receives STE100 warnings and
uses its judgment to fix valid findings. Checker or setup failures cannot pass
as clean checks.
...
to STE100. If all inputs pass, run Vale on the original text. Print each
checker's native output with a source and checker label. Use a nonzero status
for blocking findings or tool failure.
...
original file location, and Vale does not run if any STE100 check fails.
