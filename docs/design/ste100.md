...
Keep only `SKILL.md`, the nested `ste100/SKILL.md`, `prose-check`, `.vale.ini`,
`context_glossary.py`, `shared-terms.json`, and Vale styles in the installable directory. Move tests
and design notes to repository-level directories. The default `npx skills`
discovery reports `concise-prose` as one skill. Verify an actual temporary
install before relying on the nested file and dotfiles being packaged.
